...
CDNConnectClient.prototype.mkdir = function(cdnPath, newFolder, next)
{
    //FIXME: should create the full path if parent folders do not exist
    //POST https://api.cdnconnect.com/v1/bidaway.cdnconnect.com/<container_folder>/new-folder.json?folder_name=<new_folder_name>

    var self = this;
    self.performAction('POST', cdnPath, 'new-folder', {folder_name: newFolder}, function(err, data) {
        if (next) {
            next(err, data);
        }
    });
